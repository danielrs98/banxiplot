...
#' @description The function "grafica_ribbon" For each x value, displays a "y" interval defined by ymin and ymax
...
#' @param size_line line size, numeric value
#' @param size_point point size, numeric value
...
#' @param point If point = T, then add a point to the line graph
...
#' @param add_ribbon If add_ribbon = T, then ribbon format will be added to a time series
#' @param y_axis_r variable to be plotted for ribbon interval
#' @param ribbon_col color of the Ribbon area
#' @param ribbon_alpha ribbon opacity
#' @param rint Value that is subtracted from y_axis_r to obtain the interval defined by ymin and ymax
...
#' @return Ribbon plot with Banxico format
#' @references Ggplot2: Elegant Graphics for Data Analysis
#' @author Marcos Daniel Ramirez Santiago, \email{danielrs98@@outlook.com}
#' @export

grafica_ribbon <- function(df,x_axis,y_axis,fill_C,is_date=F,is_num=F,
                           is_word=F,size_line=1.5,size_point=2,
                           ...
                           point=F,line_v=F,x_int,f_wrap=F,f_wrap_1,scales_fw=T,
                           add_ribbon=F,y_axis_r,ribbon_col="grey70",ribbon_alpha=1,
                           rint,size_t=9,size_s=9,
                           ...
                           vline_alpha=1,add_text=F,
                           n_decimal=2,size_text=3.5,font_text="plain",
                           textp_y=0.04,text_hjust=-0.25,escala=F,ay,by){

  g<-ggplot(df,aes(x = {{x_axis}}, y = {{y_axis}}, color = {{fill_C}}))
  g+geom_line(size = size_line,alpha=line_alpha) + #grafica de linea
    {if(point)geom_point(size = size_point)}+
    scale_fill_manual(values = colores) +
    {if(line_v)geom_vline(xintercept=x_int,linetype = "longdash",
                          colour=vline_col,alpha=vline_alpha)}+
    {if(add_ribbon)geom_ribbon(aes(ymin = {{y_axis_r}}-rint, ymax = {{y_axis_r}}+rint),
                               fill = ribbon_col,alpha = ribbon_alpha,colour = NA)}+
    {if(add_text)geom_text(aes(label=round({{y_axis}},n_decimal)),
                           size=size_text,family="Calibri",show.legend = FALSE,
                           fontface=font_text,nudge_y =textp_y,hjust=text_hjust)}+
    theme_bw()+
    ...
    {if(f_wrap)facet_wrap(vars({{f_wrap_1}}),ncol=1,
