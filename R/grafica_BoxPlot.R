...
#' @param bigotes T
#' @param prom If average = T, then the average of the data is calculated
...
#' @param box_alpha boxPlot opacity
...
grafica_BoxPlot <- function(df,x_axis,y_axis,color_bp=colores[2],
                            color_outlier="#FDFEFE",legend_position="right",
                            legend_title="",x_t="",y_t="",y_position="right",
                            titulo="",subtitulo="",t_hjust=0.0,s_hjust=0.0,
                            f_wrap=F,f_wrap_1,scales_fw=T,bigotes=F,
                            prom=T,size_t=9,size_s=9,
                            ...
                            x_axis_f="plain",y_axis_f="plain",box_alpha=1){

  g<-ggplot(df,aes(x = {{x_axis}}, y = {{y_axis}}))
  g+geom_boxplot(fill=color_bp, outlier.colour =color_outlier,alpha=box_alpha) +
    {if(prom)stat_summary(fun="mean",size = 0,color=color_bp)}+
