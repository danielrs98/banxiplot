#' Line Plot
#' @description The function "grafica_lineas" create a line plot for multiple series in a dataset
...
#' @param i select a color
#' @param point=F If point = T, then add a point to the line graph
#' @param vline_col color of vertical line
#' @return Line plot with Banxico format
#' @references Ggplot2: Elegant Graphics for Data Analysis
#' @author Marcos Daniel Ramirez Santiago, \email{danielrs98@@outlook.com}
#' @export

grafica_lineas <- function(df,x_axis,y_axis,fill_C,size_line=1.5,size_point=2,legend_position="right",
                           legend_title="",x_t="",y_t="",y_position="right",formato_fecha="%m-%Y",
                           titulo="",subtitulo="",break_x="8 week",t_hjust=0.0,s_hjust=0.0,
                           point=F,line_v=F,x_int,f_wrap=F,f_wrap_1,scales_fw=T,
                           add_line=F,i=2,ext_line,vline_col="black"){

  g<-ggplot(df,aes(x = {{x_axis}}, y = {{y_axis}}, color = {{fill_C}}))
  g+geom_line(size = size_line) + #grafica de linea
    {if(point)geom_point(size = size_point)}+
    scale_fill_manual(values = colores) +
    {if(line_v)geom_vline(xintercept=x_int,linetype = "longdash",colour=vline_col)}+
    {if(add_line)geom_line(aes(y = {{ext_line}}), color = colores[i], size = 1)}+
    theme_bw()+
