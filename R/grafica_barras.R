#' Bar Plot
#' @description The function "grafica_barras" create a bar plot for multiple series in a dataset
...
#' @param position_bar stack or dodge bars
...
grafica_barras <- function(df,x_axis,y_axis,fill_C,position_bar="stack",
                           ...
                           f_wrap=F,f_wrap_1,scales_fw=T,line_v=F,x_int,
                           add_line=F,ext_line,i=2,vline_col="black"){

  g<-ggplot(df,aes(x = {{x_axis}}, y = {{y_axis}}, fill = {{fill_C}}))
  g+geom_bar(stat="identity", position=position_bar) +
